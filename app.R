...
server <- function(input, output) {
	...
	output$temp_plot <- renderPlot({
		...
		ggplot(temp_df) +
			geom_ribbon(aes(x=time, ymin=temp-2*std, ymax=temp+2*std), alpha=0.3, fill="orange") +
			geom_ribbon(aes(x=time, ymin=temp-std, ymax=temp+std), alpha=0.3, fill="orange") +
			geom_line(aes(x=time, y=temp), size=2, colour="orange") +
	...
	draw_growth_plot <- function(pop) {
		...
		if (length(pop) == 0) {
			...
			output$growth_plot <- renderPlot({
				ggplot(sim_df) +
					geom_line(aes(x=time, y=pop_saved), size=1, alpha=0.5, linetype="dashed") +
					geom_line(aes(x=time, y=pop), size=1) +
